...
def node_finalize_answer(state: GraphState) -> GraphState:
    """Step 4: return the sanitized SQL preview with placeholder rows + bar chart.
    ...
    state["response"] = ChatResponse(
        ...
        narrative=(
            ...
            "(SQL generated and sanitized — real execution arrives in step 5.)"
        ),
...
def _route_after_sanitize(state: GraphState) -> str:
    return "blocked_sql" if state.get("sanitize_blocked") else "finalize_answer"
